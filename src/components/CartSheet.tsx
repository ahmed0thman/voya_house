...
import { useWhatsappOrderNumber } from "@/hooks/use-settings";
import {
  EMPTY_CONTACT,
  readGuestContact,
    ...
    <div className="mb-3">
      ...
      <div
        className={`flex items-center gap-2 px-3 rounded-xl bg-black/30 border transition-all ${
          error
            ? "border-red-500/60"
            : "border-white/10 focus-within:border-[#F1E6C3]"
        }`}
      >
        ...
        >
          ...
          {tables.map((table) => (
            <option key={table.number} value={table.number}>
              {tCommon("tableNumber", {
                number: formatTableDigits(table.number),
              })}
      ...
      {error && (
        <span className="block px-1 mt-1 text-[10px] text-red-400 font-mono">
          {error}
        </span>
      )}
    ...
    { label: mode("readyStepLabel"), hint: mode("readyStepHint") },
    {
      ...
      hint:
        order.type === "ON_TABLE" && order.tableNumber !== null
          ? t("progress.toTable", {
              table: tCommon("tableNumber", {
                number: formatTableDigits(order.tableNumber),
              }),
            })
          : mode("finalStepHint"),
    },
  ];
  const currentIndex = PROGRESS_ORDER.indexOf(
    order.status as (typeof PROGRESS_ORDER)[number],
  );
...
export default function CartSheet() {
  ...
  /** Unset until an admin configures one in Settings — the WhatsApp handoff below is skipped until then. */
  const { data: whatsappOrderNumber } = useWhatsappOrderNumber();
  ...
  const validateContact = (): typeof fieldErrors => {
    ...
    if ((contact.phone.match(/\d/g)?.length ?? 0) < 7)
      next.phone = tValidation("phone");
  ...
  const handlePlaceOrder = (e: React.FormEvent) => {
    ...
    // Opened synchronously, inside the click's own handler — a browser only
    // exempts window.open from its popup blocker when it's called this way.
    // Doing it after the mutation resolves (an async gap) gets silently
    // blocked in most browsers, so we open a blank tab now and point it at
    // the real wa.me URL once the order comes back.
    const whatsappTab = whatsappOrderNumber ? window.open("", "_blank") : null;
    ...
    placeOrderMutation.mutate(input, {
      onSuccess: (newOrder) => {
        ...
        if (whatsappOrderNumber && whatsappTab) {
          whatsappTab.location.href = buildWhatsappOrderUrl(
            whatsappOrderNumber,
            newOrder,
          );
        } else {
  ...
  /** Mirrors `codeSchema` so the guest sees a localized rule instead of the server's raw (English) Zod message. */
  const isValidOfferCodeShape = (value: string) =>
    value.trim().length >= 3 && /^[A-Za-z0-9_-]+$/.test(value.trim());

  const handleApplyOfferCode = () => {
    const trimmed = offerCodeInput.trim();
    if (!trimmed) return;
    if (!isValidOfferCodeShape(trimmed)) {
      toast.error(tValidation("offerCode"));
      return;
    }
      ...
      onSuccess: (offer) => {
        ...
        toast.success(
          offer.discountType === "PERCENT"
            ? t("offerPercentApplied", { percent: offer.discountValue })
            : t("offerAmountApplied", {
                amount: formatPrice(offer.discountValue),
              }),
        );
      },
  ...
  return (
    ...
    >
              ...
              {orderMode && (
                <span className="px-2 py-0.5 rounded-full border border-[#F1E6C3]/30 bg-[#F1E6C3]/10 font-mono text-[9px] sm:text-[10px] text-[#F1E6C3] font-bold">
                  {orderMode === "ON_TABLE" && tableNumber !== null
                    ? tCommon("tableNumber", {
                        number: formatTableDigits(tableNumber),
                      })
                    : tMode(orderMode)}
                </span>
              )}
          ...
          <div className="p-1 rounded-2xl bg-white/[0.05] border border-white/10 grid grid-cols-2 gap-1 font-mono text-xs">
            ...
            >
              ...
              <span className="relative flex h-2 w-2">
                <span
                  className={`animate-ping absolute inline-flex h-full w-full rounded-full ${showOrderStatus ? "bg-black" : "bg-[#B7D39A]"} opacity-75`}
                />
                <span
                  className={`relative inline-flex rounded-full h-2 w-2 ${showOrderStatus ? "bg-black" : "bg-[#B7D39A] shadow-[0_0_8px_rgba(183,211,154,1)]"}`}
                />
              </span>

              <Clock01Icon
                size={13}
                className={showOrderStatus ? "text-black" : "text-[#F1E6C3]"}
              />
              <span>{t("myOrders", { count: activeOrders.length })}</span>
            </button>
          </div>
        </div>
      )}
      ...
      <div
        ref={contentRef}
        className="w-full max-w-4xl mx-auto py-3 sm:py-6 flex-1 flex flex-col justify-center"
      >
          ...
          <div className="cart-anim-item flex flex-col items-center max-w-2xl mx-auto w-full text-center">
              ...
              <div className="w-full flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide mb-4">
                ...
                {activeOrders.map((order, idx) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`w-2 h-2 rounded-full ${isSelected ? "bg-black" : "bg-[#B7D39A]"} animate-pulse`}
                      />
                      <span>{t("round", { number: roundNum })}</span>
            ...
            <div className="w-full bg-white/[0.03] border border-white/10 rounded-2xl p-4 sm:p-6 mb-5 sm:mb-8 text-start shadow-xl">
                ...
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-lg bg-[#F1E6C3]/20 text-[#F1E6C3] flex items-center justify-center font-bold text-xs">
                    {activeOrders.findIndex(
                      (o) => o.id === activeSelectedOrder.id,
                    ) + 1}
                  </div>
                  <div>
                    <h3 className="font-serif text-lg sm:text-xl text-white font-medium">
                      {t("ticket", {
                        id: formatTicketId(activeSelectedOrder.id),
                      })}
                    </h3>
                    <span className="font-mono text-[10px] text-white/50">
                      {t("placedAt", {
                        time: formatOrderTime(activeSelectedOrder.createdAt),
                      })}{" "}
                      ·{" "}
                      {activeSelectedOrder.tableNumber !== null
                        ? tCommon("tableNumber", {
                            number: formatTableDigits(
                              activeSelectedOrder.tableNumber,
                            ),
                          })
              ...
              <div className="pt-3 sm:pt-4 border-t border-white/10 space-y-2.5">
                <span className="font-mono text-[9px] uppercase tracking-wider text-[#F1E6C3] font-bold block">
                  {t("itemsInTicket", {
                    id: formatTicketId(activeSelectedOrder.id),
                    count: activeSelectedOrder.items.reduce(
                      (s, i) => s + i.quantity,
                      0,
                    ),
                  })}
                ...
                {activeSelectedOrder.items.map((it) => (
                  <div
                    key={it.id}
                    className="flex justify-between items-center text-xs text-white/90"
                  >
                    <div className="flex items-center gap-2">
                  ...
                  <div className="pt-2 border-t border-white/5 space-y-1 text-[11px] text-white/50">
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono">
                        {formatPrice(activeSelectedOrder.subtotal)}
                      </span>
                    </div>
                    <div className="flex justify-between text-[#B7D39A]">
                      <span>
                        {t("discount")}
                        {activeSelectedOrder.offerCode
                          ? ` (${activeSelectedOrder.offerCode})`
                          : ""}
                      </span>
                      <span className="font-mono">
                        -{formatPrice(activeSelectedOrder.discountAmount)}
                      </span>
                    </div>
                  </div>
                )}
              ...
              {activeSelectedOrder.specialNotes && (
                <div
                  className="mt-3 pt-3 border-t border-white/5 text-[11px] text-white/60 italic"
                  dir="auto"
                >
                  {t("note", { note: activeSelectedOrder.specialNotes })}
                </div>
              )}
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="font-serif text-sm text-white font-medium">
                                {t("ticket", { id: formatTicketId(order.id) })}
                              </span>
                              <span
                      ...
                      {isExpanded && (
                        <div className="mt-3 pt-3 border-t border-white/5 space-y-1.5">
                          {order.items.map((it) => (
                            <div
                              key={it.id}
                              className="flex justify-between items-center text-xs text-white/70"
                            >
                              <span dir="auto">
                                {it.quantity}× {it.name}
                              </span>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full justify-center">
              ...
              {hasItems && (
                ...
                >
                  {t("reviewNewCart", { count: totalItems })}
                </button>
              )}
            </div>
          </div>
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <h4
                              className="font-serif text-sm sm:text-base text-white font-medium truncate max-w-[160px] sm:max-w-[200px]"
                              dir="auto"
                            >
                              {item.name}
                            </h4>
                          ...
                          <span className="font-mono text-[11px] text-white/50 hidden sm:inline">
                            {t("priceEach", { price: formatPrice(item.price) })}
                          </span>
                        </div>
                      </div>
            ...
            <div className="lg:col-span-5 rounded-2xl border border-white/15 bg-white/[0.04] p-4 sm:p-6 backdrop-blur-xl flex flex-col justify-between shadow-2xl">
              <div>
                      ...
                      {appliedOffer ? (
                        <div className="flex items-center justify-between p-2.5 rounded-xl bg-[#B7D39A]/10 border border-[#B7D39A]/30">
                          <div className="flex items-center gap-2 min-w-0">
                            <DiscountTag01Icon
                              size={14}
                              className="text-[#B7D39A] shrink-0"
                            />
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            disabled={
                              !offerCodeInput.trim() ||
                              validateOfferMutation.isPending
                            }
                ...
                <div className="space-y-1.5 pt-2.5 border-t border-white/10 text-xs text-white/70">
                  <div className="flex justify-between">
                    <span>{t("itemsSummaryLabel", { count: totalItems })}</span>
                    <span className="font-mono">{formatPrice(totalPrice)}</span>
                  </div>
                  {appliedOffer && (
                    <div className="flex justify-between text-[#B7D39A]">
                      <span>
                        {t("discount")} ({appliedOffer.code})
                      </span>
                      <span className="font-mono">
                  ...
                  <div className="hidden flex justify-between">
                    ...
                    <span className="font-mono text-[#B7D39A]">
                      {t("included")}
                    </span>
                  </div>
