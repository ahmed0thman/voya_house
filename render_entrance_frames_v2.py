...
def render_scene1(output_dir="public/assets/frames", total_frames=78):
    ...
    # Inpaint / clean the hallway behind doors
    interior_clean = cv2.GaussianBlur(entrance_img, (25, 25), 0)
    interior_bright = cv2.convertScaleAbs(interior_clean, alpha=1.3, beta=30)
    gold_tint = np.full_like(interior_bright, (40, 155, 255), dtype=np.uint8)
    ...
    for f in range(1, total_frames + 1):
        ...
        if door_t > 0.001:
            ...
            facade[door_outer_y1:door_outer_y2, door_outer_x1:door_outer_x2] = \
                interior_lit[door_outer_y1:door_outer_y2, door_outer_x1:door_outer_x2]
            ...
            # Keep door outer casing (top lintel, threshold, outer sides) perfectly sharp
        ...
        if f >= flare_start:
            ...
            # Blend smoothly into original video transition on frames 74..78
            if f >= 74:
